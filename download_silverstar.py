"""
Download Silver Star aerial imagery from BC Government WMS
============================================================
Downloads 1m resolution orthophoto tiles covering the Silver Star
Mountain residential areas (Silver Queen/Monashee) near Vernon, BC.

Saves as georeferenced GeoTIFFs in EPSG:32610 (UTM Zone 10N).

Usage:
    python download_silverstar.py

Output:
    ./silver_star_imagery/  (GeoTIFF files)
"""
...
# Output directory
OUT_DIR = Path("/home/jeremy/COMP4900_Project_backup_files/silver_star_imagery")

# Silver Star Mountain, BC bounds (lat/lon)
# Covers Silver Queen, Monashee residential subdivisions
AREA_BOUNDS = {
    "west": -119.10,
    "east": -119.04,
    "south": 50.34,
...
def save_geotiff(img_data, filepath, lon_min, lat_min, lon_max, lat_max):
    ...
    ul_e, ul_n = latlon_to_utm10n(lon_min, lat_max)
    lr_e, lr_n = latlon_to_utm10n(lon_max, lat_min)
...
def main():
    ...
    print(f"Silver Star WMS Imagery Downloader")
    print(f"===================================")
    ...
    for lon_idx in range(lon_steps):
        for lat_idx in range(lat_steps):
            ...
            tile_name = f"ss_{lon_idx:03d}_{lat_idx:03d}.tif"
