"""
Download West Kelowna aerial imagery from BC Government WMS
============================================================
Downloads 1m resolution orthophoto tiles covering the main
residential/WUI areas of West Kelowna, BC.

Saves as georeferenced GeoTIFFs in EPSG:32610 (UTM Zone 10N),
compatible with the existing parcel_risk.py pipeline.

Usage:
    python download_westkelowna.py

Output:
    ./west_kelowna_imagery/  (GeoTIFF files)
"""
...
# Output directory
OUT_DIR = Path("./west_kelowna_imagery")

# West Kelowna WUI residential areas (lat/lon bounds)
# Covers: Westbank, Shannon Lake, Glenrosa, Smith Creek, Rose Valley
AREA_BOUNDS = {
    "west": -119.66,
    "east": -119.48,
    "south": 49.82,
...
# ~0.01 deg lon ≈ 720m, ~0.01 deg lat ≈ 1110m
# At 1m resolution with 1024px tiles: ~1km per tile
TILE_SIZE_DEG = 0.01
...
def latlon_to_utm10n(lon, lat):
    ...
    Uses simple formulae — accurate enough for georeferencing at 1m."""
    import math
    a = 6378137.0
    f = 1 / 298.257223563
    e2 = 2 * f - f * f
...
def main():
    ...
    print(f"West Kelowna WMS Imagery Downloader")
    print(f"====================================")
    ...
    for i, lon_idx in enumerate(range(lon_steps)):
        for lat_idx in range(lat_steps):
            ...
            tile_name = f"wk_{lon_idx:03d}_{lat_idx:03d}.tif"
    ...
    # Print GSD info
    print(f"\nNote: This imagery is ~1m GSD (ground sample distance).")
    print(f"Set --gsd 1.0 when running the risk assessment pipeline.")
